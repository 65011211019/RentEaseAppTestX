...
export const createProduct = async (ownerId: number, productData: Omit<Product, 'id' | 'owner_id' | 'slug' | 'created_at' | 'updated_at' | 'owner' | 'province' | 'category' | 'primary_image' | 'view_count' | 'admin_approval_status' | 'images'> & { images?: File[] }): Promise<Product> => {
    try {
        const token = localStorage.getItem('authToken');
        if (!token) {
            throw new Error('No authentication token found');
