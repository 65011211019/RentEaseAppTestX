...
export const login = async (credentials: LoginCredentials): Promise<LoginResponse> => {
  try {
    const response = await axios.post<ApiResponse<LoginResponse>>('https:/renteaseapptestapi.onrender.com/api/auth/login', credentials, {
      headers: {
...
export const register = async (credentials: RegisterCredentials): Promise<RegisterResponse> => {
  try {
    const response = await axios.post<ApiResponse<{ user: User; access_token: string }>>('https:/renteaseapptestapi.onrender.com/api/users/register', credentials);
...
export const forgotPassword = async (email: string): Promise<ForgotPasswordResponse> => {
  try {
    const response = await axios.post('https:/renteaseapptestapi.onrender.com/api/auth/request-password-reset', { email });
...
export const resetPassword = async (payload: ResetPasswordPayloadType): Promise<ResetPasswordResponse> => {
  try {
    const response = await axios.post('https:/renteaseapptestapi.onrender.com/api/auth/reset-password-with-otp', payload, {
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
      }
